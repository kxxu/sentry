 ...
 * will be cached via the infrastructure provided by {@link org.apache.hadoop.security.Groups}.
 *
...
/**
 * Created by kxxu on 2016/9/20.
 * modify setConf Null Exception
 */
...
//            LOG.warn("ldap get groups, user: " + user);
    ...
    public synchronized void setConf(Configuration conf) {
        ...
        Collection providerPath = conf.getStringCollection("hadoop.security.credential.provider.path");
        if (providerPath != null && providerPath.size() > 0) {
            LOG.warn("password provider path: " + conf.getStringCollection("hadoop.security.credential.provider.path"));
        }
    ...
    String getPassword(Configuration conf, String alias, String defaultPass) {
        ...
        try {
            ...
            LOG.warn("Exception while trying to password for alias " + alias + ": "
                    + ioe.getMessage());
            LOG.warn("LDAP group mapping got Exception: " + Throwables.getStackTraceAsString(ioe));
        }
//        if (password == null) {
//            password = defaultPass;
//        }
    ...
    String extractPassword(String pwFile) {
        if (Strings.isNullOrEmpty(pwFile)) {
            return "";
        }
//        if (pwFile.isEmpty()) {
